...
File2hash("$FindBin::Bin/kt.ini",\%ktini);  #Load directory info into $ktini

# Make the archived site portable when served from this checkout.
$ProjectRoot = "$FindBin::Bin/..";
$ktini{engine} = "/cgi-bin/kt.cgi";
$ktini{enginektdb} = "$FindBin::Bin/kt.db";
foreach $section (qw(home toystore school township museum cityhall citypark zoo library)) {
        $ktini{"${section}graphics"} = "/graphics/$section";
        $ktini{"${section}scripts"} = "$ProjectRoot/scripts/$section";
        $ktini{"${section}data"} = "$ProjectRoot/data/$section";
}
